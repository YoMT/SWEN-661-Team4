...
enum ReportPeriod { last7, last30, last90, all }

extension ReportPeriodLabel on ReportPeriod {
  String get label => switch (this) {
        ReportPeriod.last7 => 'Last 7 days',
  ...
  Duration? get window => switch (this) {
        ReportPeriod.last7 => const Duration(days: 7),
        ReportPeriod.last30 => const Duration(days: 30),
        ReportPeriod.last90 => const Duration(days: 90),
        ReportPeriod.all => null,
      };
}

class CaretakerNotesScreen extends StatefulWidget {
  const CaretakerNotesScreen({super.key});

  @override
  State<CaretakerNotesScreen> createState() => _CaretakerNotesScreenState();
}

class _CaretakerNotesScreenState extends State<CaretakerNotesScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final cutoff = _period.window == null
        ? null
        : DateTime.now().subtract(_period.window!);
    final visibleNotes = cutoff == null
        ? caretaker.notes
        : caretaker.notes.where((n) => n.createdAt.isAfter(cutoff)).toList();
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const Text('Notes from care team',
              style: TextStyle(
          ...
          if (visibleNotes.isEmpty)
            const Padding(
              ...
              child: Text('No notes in this period',
                  style: TextStyle(fontSize: 14, color: AppColors.textMuted)),
            )
          else
            for (final note in visibleNotes) NoteCard(note: note),
