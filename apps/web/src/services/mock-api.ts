// In-memory mock backend, ported verbatim (behaviour-wise) from apps/mobile.
// Keeps the desktop app fully functional with no server running.
import type {
  User,
  Profile,
  Medication,
...
const hoursAgo = (n: number): string => new Date(Date.now() - n * 3600000).toISOString()

const daysFromNow = (n: number, h = 10, m = 0): string => {
  const d = new Date()
...
const _contacts: EmergencyContact[] = [
  {
    id: 'c1',
    name: 'Sarah Johnson',
    phone: '(555) 234-5678',
...
export function mockRequest<T>(method: string, path: string, body?: unknown): T {
  ...
  if (method === 'PATCH' && replyMatch) {
    ...
    _notes = _notes.map((n) => (n.id === id ? { ...n, replyContent: reply, updatedAt: ts } : n))
